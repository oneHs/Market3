...
function deleteBill(obj) {
	$.ajax({
		type : "GET",
		// path + "/bill/billDel.action?billid=" + obj.attr("billid")
...
//				window.location.href = path + "/bill/billPage.action";
				 window.location.href =  "../bill/billPage.action";
...
$(function() {
	$(".viewBill").on(
			"click",
			function() {// on函数 用来绑定事件
				// 将被绑定的元素（a）转换成jquery对象，可以使用jquery方法
				var obj = $(this);
				window.location.href = path
						+ "/bill/billShowAll.action?billid="
						+ obj.attr("billid");
			});
	...
	$(".modifyBill").on(
			...
			function() {
				var obj = $(this);
				window.location.href = path + "/bill/billDetail.action?billid="
						+ obj.attr("billid");
			});
	$('#no').click(function() {
		cancleBtn();
	...
	/*
	 * $(".deleteBill").on("click",function(){ var obj = $(this);
	 * if(confirm("你确定要删除订单【"+obj.attr("billcc")+"】吗？")){ $.ajax({ type:"GET",
	 * url:path+"/bill.do", data:{method:"delbill",billid:obj.attr("billid")},
	 ...
	 * alert("对不起，删除订单【"+obj.attr("billcc")+"】失败"); }else if(data.delResult ==
	 * "notexist"){ alert("对不起，订单【"+obj.attr("billcc")+"】不存在"); } },
